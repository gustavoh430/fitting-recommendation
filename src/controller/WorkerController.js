...
export class WorkerController {
    ...
    setupCallbacks() {
        ...
        this.#worker.onmessage = (event) => {
            ...
            if (event.data.type === workerEvents.recommend) {
                console.log("dispatchRecommendationsReady", event.data.recommendations.length); this.#events.dispatchRecommendationsReady(event.data);
            }
        };
    }
    ...
    triggerRecommend(user) {
        console.log("Sending recommend to worker!"); this.#worker.postMessage({ action: workerEvents.recommend, user });
    }
}